...
function ruleLabel(r){return DAYS[r.day]+(r.nth?' ke-'+r.nth:' setiap minggu')+' · '+r.type;}
...
function bindProfiles(){
 ...
 $('ruleDay').innerHTML=[1,2,3,4,5,6,0].map(d=>`<option value="${d}">${DAYS[d]}</option>`).join('');$('ruleForm').onsubmit=saveRule;$('newRuleBtn').onclick=()=>editRule();$('ruleItems').onclick=e=>{const b=e.target.closest('[data-rule]');if(b)editRule(b.dataset.rule);};
